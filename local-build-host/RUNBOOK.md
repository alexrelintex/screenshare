...
| `bind: address already in use` on `make up` | something else already owns that host port | `make ports` shows the holder. Then either kill it, or put `WEB_PORT=3100` in `.env` — compose and the Makefile both read it |
| `make ports` / `make smoke` uses an empty port | in a Makefile, `export FOO` marks FOO **defined-but-empty**, so a later `FOO ?= 8000` never fires | put `?=` defaults **before** the `export` line |
| `smoke.sh` reports `Could not resolve host: services` | a bare `export` in the Makefile pushed *every* make variable into the script, and `API := services/api` clobbered the script's `$API` URL | export only the specific variables you mean |
